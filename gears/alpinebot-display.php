 ...
 */
  function set_size_id(){
    $this->set_active_option('size_id','/236'); // Default is 236
          ...
          if( count($photos)<$options['pinterest_photo_number'] ){
            ...
            if( !empty($url) && !empty($content) ){
              ...
              if( !empty( $matches[0] ) ){
                ...
                $the_photo['image_source'] = @str_replace('/236/', $size_id.'/', $photourl_current);

                //if( $size_id == '/75' &&  (strpos($the_photo['image_source'],'.pinimg.com') === false)){
                if( $size_id == '/75x75' ){
                  $the_photo['image_source'] = @str_replace('/236x/', '/75x75/', $the_photo['image_source']);
                }else{
                  $the_photo['image_source'] = @str_replace('/236x/', $size_id.'x/', $the_photo['image_source']);
                }

                $the_photo['image_original'] = @str_replace('/236/','/600/', $photourl_current);
                $the_photo['image_original'] = @str_replace('/236x/','/600x/', $the_photo['image_original']);
...
/**
 *  Function for printing carousel style
 *
 *  @ Since 0.0.1
 *  @ Updated 1.2.6.2
 */
 function display_gallery(){
   ...
   $src = $this->get_private('src');
   $wid = $this->get_private('wid');
   if($opts['radius_ex'] == "percent"){
     $border_r_t = "%";
   }
   elseif($opts['radius_ex'] == "pixel"){
       $border_r_t = "px";
   }
   else {
     $border_r_t = "px";
...
.grid-con .item {
    ...
    width: 100%!important;
    height: <?php echo $opts['style_photo_height'] . $h ;?>;
}
.grid-con{

 display: grid;
 grid-gap: 10px;
grid-template-columns: repeat(<?php echo $opts['style_photo_per_row']; ?>,1fr);
}
</style>
<?php
   $this->add('<div id="'.$wid.'-AlpinePhotoTiles_container" class="">');

     // Align photos
     $css = $this->get_parent_css();
     $this->add('<div id="'.$wid.'-vertical-parent" class="AlpinePhotoTiles_parent_class" style="'.$css.'">');

$this->add('<div class="row grid-container" >');
$this->add('<div class="container viewdivv column ">');
$this->add('<img id="exp" style="width:100%;padding:2px;">');

$this->add('</div>');
$this->add('<div class=" grid-con column">');

      for($i = 0;$i<$opts[$src.'_photo_number'];$i++){
        $css = "margin:1px 0 5px 0;padding:0;max-width:100%;";
        $pin = $this->get_option( 'pinterest_pin_it_button' );
        $this->add_image_gallery($i,$pin); // Add image
      }
...
<style>
<?php echo "#".$wid ?> img.AlpinePhotoTiles-img-corners {
  border-radius: <?php echo $opts['border_radius_textbox']. "".$border_r_t; ?>
}
</style>
<script>
...
function display_carousel(){
  ...
  $equal_height ="";
if( $opts['equal_height']=='true')
{  $equal_height= 'AlpinePhotoTiles_carousel'; }

    $this->add('<div id="'.$wid.'-carousel-parent" class=" '.$equal_height.' AlpinePhotoTiles_parent_class" style="'.$css.'">');
    // Align photos
    $css = $this->get_parent_css();
...
jQuery('#<?=$wid?>').owlCarousel({

  margin:<?php echo $opts['image_margin']; ?>,
  responsiveClass:true,

  responsive:{
      0:{
          items:<?php echo $opts['mob_column_number']; ?>,
      },
      600:{
          items:<?php echo $opts['tab_column_number']; ?>,
      },
      1000:{
          items:<?php echo $opts['style_column_number']; ?>,
      }
  },
  autoplay:<?php echo $opts['auto_play']; ?>,
  autoplayTimeout:<?php echo $opts['autoplaytimeout']; ?>,
  autoplayHoverPause:true,

  loop:true,
 nav:<?php echo $opts['navigation']; ?>,
 items:<?php echo $opts['style_column_number']; ?>,
...
<?php
$this->add('<ul id="'.$wid.'" class="owl-carousel owl-theme AlpinePhotoTiles-owl-item ">');


      for($i = 0;$i<$opts[$src.'_photo_number'];$i++){
        $css = "margin:1px 0 5px 0;padding:0;max-width:100%;";
        $pin = $this->get_option( 'pinterest_pin_it_button' );
        $this->add_image_carousel($i,$css,$pin); // Add image
      }
 ...
 */
  function display_hidden(){
    ...
    $this->add('<div id="'.$wid.'-carousel-parent" class="AlpinePhotoTiles_parent_class" style="'.$css.'">');  // Align photos
 ...
 */
  ...
  function add_image_carousel($i,$css="",$pin=false){
    $light = $this->get_option( 'general_lightbox' );
    $title = $this->get_photo_info($i,'image_title');
    $src = $this->get_photo_info($i,'image_source');
    $shadow = ($this->check_active_option('style_shadow')?'AlpinePhotoTiles-img-shadow':'AlpinePhotoTiles-img-noshadow');
...
<script>
jQuery(document).ready(function() {
 jQuery('.btn-gallery').magnificPopup({
  type: 'image',
  gallery: {
    enabled:true
  }
        ...
        if( $pin ){ $this->add('<li class=" AlpinePhotoTiles-pinterest-container" style="position:relative;display:block;" >'); }
   ...
   $has_link = $this->get_link($i); // Add link
    $this->add('<img id="'.$this->get_private('wid').'-tile-'.$i.'" class="AlpinePhotoTiles-image '.$shadow.' '.$border.' '.$curves.' '.$highlight.'" src="' . $src . '" ');
    $this->add('title='."'". $title ."'".' alt='."'". $title ."' ");
     // Caecho reful about caps with ""
  ...
  function add_image_gallery($i,$css="",$pin=false){
    //$light = $this->get_option( 'general_lightbox' );
    $title = $this->get_photo_info($i,'image_title');
    $src = $this->get_photo_info($i,'image_source');
    $shadow = ($this->check_active_option('style_shadow')?'AlpinePhotoTiles-img-shadow':'AlpinePhotoTiles-img-noshadow');
...
function m(imgs) {
  ...
  exp.parentElement.style.display = "block";
   ...
   $has_link = $this->get_link($i);

  $this->add('<img id="'.$this->get_private('wid').'-tile-'.$i.'" onload="m(this);" class="item AlpinePhotoTiles-image '.$shadow.' '.$border.' '.$curves.' '.$highlight.'" src="' . $src . '" ');// Add link

 $this->add('<img id="'.$this->get_private('wid').'-tile-'.$i.'" onclick="m(this);" class="item AlpinePhotoTiles-image '.$shadow.' '.$border.' '.$curves.' '.$highlight.'" src="' . $src . '" ');
 ...
 */
  function get_link($i){
    ...
    if( 'original' == $link && !empty($photourl) ){
      $this->add('<a href="' . $photourl . '" class=" btn-gallery AlpinePhotoTiles-link" target="_blank" title=" '. $phototitle .' " alt=" '. $phototitle .' ">');
      return true;
    }elseif( ($src == $link || '1' == $link) && !empty($linkurl) ){
      $this->add('<a href="' . $linkurl . '" class="btn-gallery AlpinePhotoTiles-link" target="_blank" title=" '. $phototitle .' " alt=" '. $phototitle .' ">');
      return true;
    }elseif( 'link' == $link && !empty($url) ){
      $this->add('<a href="' . $url . '" class="btn-gallery AlpinePhotoTiles-link" title=" '. $phototitle .' " alt=" '. $phototitle .' ">');
      return true;
    }elseif( 'fancybox' == $link && !empty($originalurl) ){
      $light = $this->get_option( 'general_lightbox' );
      $this->add('<a href="' . $originalurl . '" class="btn-gallery AlpinePhotoTiles-link AlpinePhotoTiles-lightbox" title=" '. $phototitle .' " alt=" '. $phototitle .' ">');
      return true;
    }
    return false;
  }
